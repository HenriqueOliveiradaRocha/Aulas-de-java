...
private Connection conn;
	...
	public void salvar(JButton salvar, Professor pr, Pessoa p) throws SQLException{
		String sql = "insert into professor (nome, idade, endereco, bairro, cpf,sexo, materia, instituicao) values ('"+p.getNome()+"','"+p.getIdade()+"', '"+p.getEndereco()+"', '"+p.getBairro()+"', '"+p.getCpf()+"','"+p.getSexo()+"', '"+pr.getMateria()+"','"+pr.getInstituicao()+"')";
	...
	public void alterar(JButton alterar, Professor pr, Pessoa p) {
		String sql = "update professor set nome= '"+p.getNome()+"', Idade'"+p.getIdade()+"', endereco'"+p.getEndereco()+"',bairro '"+p.getBairro()+"',cpf '"+p.getCpf()+"','"+p.getSexo()+"', materia'"+pr.getMateria()+"',instituicao '"+pr.getInstituicao()+"';";
		System.out.println(sql);
